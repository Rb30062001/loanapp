...
# Feature Scaling
dataset = pd.read_csv('loanapp.csv')
...
def main():
   ...
   <center><p style="font-size:30px;color:black;margin-top:10px;">Project</p></center> 
   <center><p style="font-size:25px;color:black;margin-top:10px;"Machine Learning</p></center> 
   </div>
   </div>
   </div>
   """
    st.markdown(html_temp,unsafe_allow_html=True)
    st.header("Loan approval Prediction using Logistic Classification")
    ...
    if st.button("Predict"):
      ...
      st.success('Model predicted {}'.format(result))
